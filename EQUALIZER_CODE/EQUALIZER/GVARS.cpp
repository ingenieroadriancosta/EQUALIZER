...
HWND HWPlay, HWStop, HWEXIT, HWFw, HWRw;
HWND EDIT_INFO;
HWND TIME_INFO;
HWND HWndVALPHA;
...
BOOL InPlt = FALSE;
int*  Indexs = NULL;
BOOL ParserInd( int iPos, unsigned short Val, unsigned short IList ){
    const int iPosT = 1023&iPos;
    const unsigned short IP2 = 1023&Val;
    BOOL Set = TRUE;
    for( int i=0; i<iPosT; i++ ){
        if( Indexs[i]==IP2 || IP2>=IList ){
            Set = FALSE;
            break;
        }
    }
